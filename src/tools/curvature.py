...
def getPrincipalCurvatures(H,K):
    ...
    k1 = torch.abs(H[pos] + torch.sqrt(s[pos]))
    k2 = torch.abs(H[pos] - torch.sqrt(s[pos]))
...
def meanCurvature(jacobian_matrix, hessian_matrix):
    ...
    num1 = torch.bmm(j.view(-1,1,3), torch.bmm(hessian_matrix, j.view(-1,3,1))).squeeze()
