...
from src.utils.utils import normalize_pts_withdia
from src.models.loss import implicit_loss
import matplotlib
...
def getImplicitCurvatureforSamples(modelnet, samples, args):
    ...
    for i in range(numbatch):
        ...
        pred_sdf_tensor = modelnet(xyz_tensor.float())
        predicted_gradient, hessian_matrix = getGradientAndHessian(pred_sdf_tensor, xyz_tensor)
        #hessloss = implicit_loss(predicted_gradient, hessian_matrix, args, device)
        #totalloss += hessloss.item()
        #hessloss.backward()
        #optimizer.step()
...
def getImplicitCurvatureforLatentSamples(model, latent, samples, args):
    gaussCurvature = np.zeros((len(samples), ))
    model.eval()
    start_idx = 0
...
def getmcubePoints(dataset, model, args, latent=None):
    ...
    for i in range(num_batch):
        ...
        with torch.no_grad():
            if latent is None:
                pred_sdf_tensor = model.net(xyz_tensor)
            else:
                latvec = latent.expand(this_bs,-1).to(device)
                newxyz = torch.cat([latvec,xyz_tensor],dim=-1).view(-1, model.latcodesize+3).to(device)
                pred_sdf_tensor = model.net(newxyz)
            #if args.clamp:
